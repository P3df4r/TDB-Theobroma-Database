...
// @ts-nocheck
const raStanza_1 = __importDefault(require("./raStanza"));
describe('RaStanza reader', () => {
    it('creates an empty stanza', () => {
        const stanza = new raStanza_1.default();
    ...
    it('parses a single line stanza', () => {
        ...
        const stanza = new raStanza_1.default(input);
        expect(stanza).toMatchSnapshot();
    ...
    it('ignores indentation with checkIndent false', () => {
        const input = '    key1 value1\n\tkey2 value\n';
        const stanza = new raStanza_1.default(input, { checkIndent: false });
        expect(stanza).toMatchSnapshot();
        expect(stanza._keyAndCommentOrder).toEqual(['key1', 'key2']);
        expect(stanza.name).toEqual('value1');
        expect(stanza.nameKey).toEqual('key1');
        expect(stanza.indent).toEqual('');
        expect(stanza.toString()).toEqual(input.replace('    ', '').replace('\t', ''));
    });
    ...
    it('throws when trying to delete the first line', () => {
        const stanza = new raStanza_1.default('key1 value1\nkey2 value2\n');
        expect(() => stanza.delete('key1')).toThrow(/Cannot delete the first line/);
    });
    it('throws with duplicate keys and different values', () => {
        expect(() => new raStanza_1.default('key1 value1\nkey1 value2\n')).toThrow(/duplicate key with a different value/);
        expect(() => new raStanza_1.default('key1 value1\nkey1 value1\n')).not.toThrow();
    });
    it('throws on encountering blank lines', () => expect(() => new raStanza_1.default('key1 value1\n\nkey2 value2')).toThrow(/contained blank lines/));
    it("throws if the first line doesn't have a value", () => expect(() => new raStanza_1.default('key1\nkey2 value2\n')).toThrow(/must have both a key and a value/));
