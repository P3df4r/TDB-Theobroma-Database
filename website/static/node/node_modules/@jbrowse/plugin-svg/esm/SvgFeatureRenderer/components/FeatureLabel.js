...
export default observer(({ text, x, y, region, reversed, bpPerPx, exportSVG, feature, viewParams, color = 'black', fontHeight = 13, featureWidth = 0, allowedWidthExpansion = 0, displayModel = {}, }) => {
    const totalWidth = featureWidth + allowedWidthExpansion;
    const measuredTextWidth = measureText(text, fontHeight);
    const params = isStateTreeNode(displayModel) && isAlive(displayModel)
    ...
    return labelVisible ? (React.createElement("text", { x: x, y: y + fontHeight, fill: color, fontSize: fontHeight }, measuredTextWidth > totalWidth
