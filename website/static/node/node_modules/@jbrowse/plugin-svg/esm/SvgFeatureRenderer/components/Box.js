...
function Box(props) {
    ...
    return feature.parent() && feature.get('type') === 'intron' ? null : (React.createElement(React.Fragment, null,
        topLevel ? React.createElement(Arrow, { ...props }) : null,
        React.createElement("rect", { "data-testid": `box-${feature.id()}`, x: leftWithinBlock, y: top, width: widthWithinBlock, height: height, fill: isUTR(feature)
                ? readConfObject(config, 'color3', { feature })
                : readConfObject(config, 'color1', { feature }), stroke: readConfObject(config, 'outline', { feature }) })));
}
